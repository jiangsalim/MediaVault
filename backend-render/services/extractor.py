...
def get_api_key(): return random.choice(YOUTUBE_API_KEYS)
...
async def search_music(query: str, platform: str = "youtube", limit: int = 25):
    ...
    methods = [("API", search_via_api), ("RSS", search_via_rss), ("yt-dlp", search_via_ytdlp)]
...
async def search_via_api(query: str, limit: int = 25):
    ...
    if "error" in data:
        ...
        async with httpx.AsyncClient() as client:
            resp = await client.get(url, params=params, timeout=10)
            data = resp.json()
        if "error" in data: return {"videos": [], "nextPageToken": ""}
    videos = []
...
async def search_via_rss(query: str, limit: int = 25):
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.get("https://www.youtube.com/feeds/videos.xml?q=" + query.replace(" ", "+"), timeout=10, follow_redirects=True)
        text = resp.text
    ...
    for entry in root.findall("atom:entry", ns)[:limit]:
        ...
        videos.append({"id": vid, "title": title, "artist": author.text if author is not None else "", "channelId": "", "description": "", "thumbnail": "https://i.ytimg.com/vi/" + vid + "/mqdefault.jpg", "publishedAt": "", "duration": 0, "views": 0, "likes": 0})
...
async def search_via_ytdlp(query: str, limit: int = 25):
    ...
    def run():
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            return ydl.extract_info("ytsearch" + str(limit) + ":" + query, download=False)
    loop = asyncio.get_event_loop()
    info = await loop.run_in_executor(None, run)
    videos = []
    for entry in info.get("entries", []) if info else []:
        if entry:
            videos.append({"id": entry.get("id", ""), "title": entry.get("title", ""), "artist": entry.get("channel", "") or entry.get("uploader", ""), "channelId": entry.get("channel_id", ""), "description": (entry.get("description", "") or "")[:200], "thumbnail": "https://i.ytimg.com/vi/" + entry.get("id", "") + "/mqdefault.jpg", "publishedAt": str(entry.get("upload_date", "")), "duration": entry.get("duration", 0) or 0, "views": entry.get("view_count", 0) or 0, "likes": entry.get("like_count", 0) or 0})
    return {"videos": videos, "nextPageToken": ""}

async def get_channel_details(channel_ids: list) -> dict:
    """Try API first, then return basic info from IDs"""
    result = {}
    # Try API
    try:
        ...
        if "error" not in data:
            for item in data.get("items", []):
                ...
                result[cid] = {"id": cid, "title": s.get("title", cid), "thumbnail": s.get("thumbnails", {}).get("medium", {}).get("url", ""), "subscriberCount": int(st.get("subscriberCount", 0)), "videoCount": int(st.get("videoCount", 0)), "customUrl": s.get("customUrl", "")}
            if result: return result
    except: pass
    
    # Fallback: return ID as name (better than showing IDs)
    for cid in channel_ids:
        result[cid] = {"id": cid, "title": "Channel " + cid[:8], "subscriberCount": 0, "thumbnail": ""}
    return result
