...
import { getRelatedSongs, getAudioStreamUrl, getDownloadUrl } from '../lib/api';
...
export default function SongModal({ song, onClose }) {
  ...
  const handlePlay = async () => {
    ...
    try {
      ...
      alert('Could not load audio preview. The song may be restricted. Try downloading instead.');
    }
  };
    ...
    <div style={{position:'fixed',inset:0,zIndex:200,background:'rgba(0,0,0,0.6)',display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={onClose}>
      <div style={{background:'#fff',borderRadius:16,maxWidth:600,width:'100%',maxHeight:'90vh',overflow:'auto',boxShadow:'0 20px 60px rgba(0,0,0,0.3)'}} onClick={e => e.stopPropagation()}>
        ...
        <div style={{textAlign:'center',padding:'0 24px 20px'}}>
          <div style={{width:120,height:68,borderRadius:8,overflow:'hidden',margin:'0 auto 16px',background:'#f0f0f0'}}><img src={thumb(song.id)} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}} onError={e=>{e.target.style.display='none'}} /></div>
        ...
        {playing && (
          <div style={{padding:'0 24px 16px'}}>
            ...
            <div style={{display:'flex',justifyContent:'space-between',fontSize:12,color:'#aaa'}}><span>{fmtTime(currentTime)}</span><span>{fmtTime(duration)}</span></div>
          </div>
        )}
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:20,padding:'0 24px 20px'}}>
          ...
          <button onClick={handlePlay} disabled={loadingAudio} style={{width:56,height:56,borderRadius:'50%',border:'none',background:'#e53935',color:'#fff',fontSize:22,cursor:'pointer'}}>{loadingAudio ? '⏳' : playing ? '⏸' : '▶'}</button>
          <button onClick={() => skip(10)} style={{width:36,height:36,borderRadius:'50%',border:'none',background:'#f0f0f0',fontSize:16,cursor:'pointer'}}>⏭</button>
        </div>

        <div style={{textAlign:'center',padding:'0 24px 8px'}}>
          <button onClick={() => setShowDownload(!showDownload)} style={{padding:'10px 20px',borderRadius:24,fontWeight:600,fontSize:14,border:'none',cursor:'pointer',background:showDownload?'#e53935':'#f0f0f0',color:showDownload?'#fff':'#333'}}>⬇ Download</button>
        </div>
        ...
        {showDownload && (
          <div style={{padding:'0 24px 20px'}}>
            <div style={{background:'#fafafa',borderRadius:12,padding:16}}>
              ...
              {[{id:'mp3',name:'MP3 128kbps',size:'~3.5 MB'},{id:'m4a',name:'M4A 128kbps',size:'~3.5 MB'}].map(f => (
              ...
              <a href={getDownloadUrl(song.id, format)} target="_blank" rel="noopener" style={{display:'block',width:'100%',marginTop:12,padding:'12px',borderRadius:8,fontWeight:600,fontSize:15,border:'none',cursor:'pointer',background:'#e53935',color:'#fff',textAlign:'center',textDecoration:'none'}}>⬇ Download MP3</a>
            </div>
          </div>
        )}
