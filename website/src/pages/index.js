...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const newReleaseQueries = ['latest ugandan music 2026','new uganda songs this week','ugandan hits today','trending uganda music'];
    ...
    Promise.all([
      ...
      // If new releases is empty, use a backup query
      if (newSongs.length === 0) {
        searchMusic('brand new ugandan music 2026').then(backup => {
          const backupSongs = (backup.data?.videos || [])
            .filter(s => !trendingIds.has(s.id))
            .slice(0, 6);
          setNewReleases(backupSongs.length > 0 ? backupSongs : allTrending.slice(6, 12));
        });
      }

      setTrending(allTrending.slice(0, 8));
      setNewReleases(newSongs.length > 0 ? newSongs : allTrending.slice(6, 12));
