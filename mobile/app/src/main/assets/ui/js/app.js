(function () {
  ...
  var Icons = {
    ...
    player: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="5 3 19 12 5 21 5 3"/></svg>',
    ...
    voice: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg>',
    ...
    theme: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/></svg>',
    ...
    share: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/></svg>',
    external: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>',
  };
  ...
  function initDrawer() {
    ...
    btn.addEventListener('click', function() { drawer.classList.add('open'); overlay.classList.add('show'); });
    overlay.addEventListener('click', function() { drawer.classList.remove('open'); overlay.classList.remove('show'); });
    ...
    var items = [
      ...
      { type: 'divider' },
      { label: 'WhatsApp Status Saver', icon: Icons.share, action: function() { Toast.show('Opening...'); } },
      { label: 'Private Vault', icon: Icons.profile, action: function() { Toast.show('Opening...'); } },
      { label: 'Phone Cleaner', icon: Icons.search, action: function() { Toast.show('Opening...'); } },
      { type: 'divider' },
      { label: 'Theme', icon: Icons.theme, action: function() { toggleTheme(); } },
      { label: 'Rate App', icon: Icons.star, action: function() { Toast.show('Rate us!'); } },
    ...
    items.forEach(function(item) {
      if (item.type === 'divider') { var d = document.createElement('div'); d.className = 'drawer-divider'; menu.appendChild(d); return; }
      if (item.href) { var a = document.createElement('a'); a.className = 'drawer-item'; a.href = item.href; a.target = '_blank'; a.innerHTML = item.icon + ' <span>' + item.label + '</span>'; menu.appendChild(a); return; }
      var b = document.createElement('button');
      b.className = 'drawer-item';
      b.innerHTML = item.icon + ' <span>' + item.label + '</span>';
      b.addEventListener('click', function() { drawer.classList.remove('open'); overlay.classList.remove('show'); item.action(); });
      menu.appendChild(b);
    });
  }
  ...
  function initBottomNav() {
    document.querySelectorAll('#bottom-nav .nav-btn').forEach(function(btn) {
      btn.addEventListener('click', function() { Router.go(this.dataset.page); });
    });
  }
  ...
  window.updateBadge = function() {
    ...
    if (active > 0) { badge.textContent = active > 99 ? '99+' : active; badge.classList.add('show'); }
